//CommentsStore负责评论对应的领域，与PostsStore相同，先创建class CommentModel，描述评价对应的state
...
class CommentsStore {
  ...
  @action fetchCommentList(postId) {
    ...
    return this.api.getCommentList(postId).then(
      action(data => {
        this.appStore.decreaseRequest();
        if (!data.error) {
          this.comments.clear();
          data.forEach(item =>
            this.comments.push(CommentModel.fromJS(this, item))
          );
  ...
  @action createComment(content) {
    this.appStore.increaseRequest();
    return this.api.createComment(content).then(
      action(data => {
        this.appStore.decreaseRequest();
        if (!data.error) {
