...
#### 怪异的装饰 ####
text-decoration不能继承。没有继承性意味着文本上画的任何装饰线(上划线或贯穿线)与父元素的颜色相同。即使后代元素本身有其他颜色也是如此。

## <font color="#ff995" face="微软雅黑" size="3">**text-shadow**</font> ##
#### 文本阴影 ####
...
## <font color="#ff995" face="微软雅黑" size="3">**white-space**</font> ##
#### 处理空白符 ####

| 值 | `normal nowrap pre pre-wrap pre-line inherit` |
| ------ | :------ |
| 初始值 | normal |
| 应用于 | <font color="#ff995" face="微软雅黑" size="3">**所有元素(CSS2.1);块级元素(CSS1和CSS2)**</font> |
| 继承性 | 无 |
| 计算值 | 根据指定确定 |

normal:使用这个属性，可以影响浏览器处理字之间和文本行之间的空白符的方式。从某种程度上讲，默认的XHTML处理已经完成了空白符处理：它会把所有空白符合并成为一个空格。所以给定normal标记，它在web浏览器中显示时，各个字之间只会显示一个空格，而且忽略元素中的换行。
normal告诉浏览器按平常的做法去做:丢掉多余的空白符。给定这个值，换行字符(回车)会转换为空格，一行中多个空格的序列也会转换为一个空格。

pre:如果设置为pre，空白符的处理就有所不同，就好像元素是XHTML pre元素一样，空白符不会被忽略。使用pre，浏览器会注意额外的空格，甚至回车。在这个方面，而且仅在这个方面，任何元素都可以相当于一个pre元素。

norwap:它会防止元素的文本换行，除非使用了一个br元素。在css中使用norwap非常类似于HTML4中用`<td norwap>`将一个表单元格设置为不能换行，不过white-space值可以应用到任何元素。

pre-wrap:该元素中的文本会保留空白符序列，但是文本行会正常地换行。如果设置为这个值，源文本中的行分隔符以及生成的行分隔符也会保留。pre-line与pre-wrap相反，会像正常文本中一样合并空白符序列，但会保留换行符。
...
## <font color="#ff995" face="微软雅黑" size="3">**direction**</font> ##
#### 文本方向 ####
...
direction属性影响块级元素中文本的书写方向，表中列布局的方向，内容水平填充其元素框的方向，以及两端对齐元素中最后一行的位置。对于行内元素，只有当unicode-bidi属性设置为embed或bidi-override时才会应用direction属性