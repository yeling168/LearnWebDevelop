...
- 对于一个非根元素，如果其position值是absolute，包含块设置为最近的position不是static的<font color="#ff995" face="微软雅黑" size="3">**祖先元素(可以是任何类型)**</font>。这个过程如下:
     ...
     --如果没有祖先，元素的包含块定义为<font color="#ff995" face="微软雅黑" size="3">**初始包含块（"根元素"的包含块(也称为初始包含块)）**</font>。
...
对于top，还会发生一些更有意思的事情。定位元素的顶端要相对于其未定位前本来的顶端位置对齐。

如果left和right设置为auto，也适用于同样的基本规则。在这些情况下，定位元素的左(或右)边界与元素未定位时该边界原本的位置对齐。

**过渡情况下忽略right的值**

>## <font color="#ff995" face="微软雅黑" size="3">**加了position之后,top,right,bottom,left才会生效**</font> ##

##<font color="#ff995" face="微软雅黑" size="3">**还要指出两个小变化。在水平布局中，如果值设置为auto、right或left都可以根据其静态位置放置。但在垂直布局中，只有top可以取静态位置，出于某种原因，bottom做不到。**</font> ##
...
非替换元素和替换元素的定位规则大不相同。这是因为替换元素有固有的高度和宽度，因此其大小不会改变。除非创作人员有意显示地修改。因此，在替换元素定位中没有"恰当收放"行为的概念。

1. 如果width设置为auto，width的实际使用值由元素内容的固有宽度决定。因此，如果一个图像的宽度是50像素，使用值则计算为50px。如果显示声明了width(也就是说，设置为100px或50%),则width设置为该值。
2. 在从左向右读的语言中，如果left值为auto，要把auto替换为静态位置。在从右向左读的语言中，则把right的auto值替换为静态位置。
3. 如果left或right仍为auto(也就是说，未在上一步中被替换)，则将margin-left或margin-right的auto值替换为0.
4. 如果此时margin-left和margin-right都还定义为auto，则把它们设置为相等的值，从而将元素在其包含块中居中。
5. 在此之后，如果只剩下一个auto值，则将其修改为等于等式的余下部分(使等式满足)。

类似的，沿垂直轴的布局受以下一组规则控制:
1. 如果height设置为auto，height的计算值由元素内容的固有高度确定。因此，对于一个50像素高的图像，其height计算为50px。如果height显式声明为某个值(如果100px或50%)，则其height会设置为该值。
2. 如果top的值为auto，则将其替换为替换元素的静态位置。
3. 如果bottom的值为auto，将margin-top或margin-bottom的所有auto值替换为0.
4. 如果此时margin-top和margin-bottom都还定义为auto，将其设置为相等的值，从而使元素在其包含块中居中。
5. 在此之后，如果仅剩下auto，则将其修改为等式中的余下部分(时等式满足)。

与非替换元素一样，如果值过度受限，用户代理会忽略bottom。

#### z轴上的放置 ####
## <font color="#ff995" face="微软雅黑" size="3">**z-index**</font> ##

| 值 | `integer auto inherit` |
| ------ | ------ |
| 初始值 | auto |
| 应用于 | <font color="#ff995" face="微软雅黑" size="3">**定位元素**</font> |
| 继承性 | 无 |
| 计算值 | 根据指定确定|

所有整数都可以作为z-index的值，包括负数。如果为元素指定一个负z-index值，会将其移到离读者更远的位置；也就是或，它会叠放栈的更底层。